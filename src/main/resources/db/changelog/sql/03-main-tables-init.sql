...
-- полное имя "ИП Белов Егор Владимирович")
-- zip - индекс юр.адреса - (обяз. поле, но не для ИП)
-- city - физ. расположение продавца (обяз. поле)
-- business_address - юр. адрес продавца полностью (обяз. поле,
-- не для ИП, г. Москва, Каширское шоссе, д55,к5)
-- inn - инн продавца (обяз. поле)
-- kpp - кпп продавца (обяз. поле, но не для ИП)
-- acc - расчетный счет продавца (куда пересылать деньги, обяз. поле)
-- phone_number - номер телефона для связи
-- email - почта для связи
-- flag_w_with_ind - флаг работают ли они с физ. лицами
-- flag_manufacturer - является ли прозводителем - проставляется исходя из роли
-- пользователя автоматически
-- rating - рейтинг
-- api_response_format - формат ответа апи, для корректной обработки
...
(
    ...
    rating              NUMERIC(3, 2),
    api_response_format VARCHAR(32),
...
INSERT INTO public.legal_entity (seller_id, bank_name, rc_bic, сorr_acc, company_name,
                                 ...
                                 email, flag_w_with_ind, flag_manufacturer, rating, api_response_format, api_address,
                                 agreement_flag)
VALUES (5, 'АО Тинькофф', '044525974', '30101810145250000974', 'OOO Электротехника', '124482', 'Москва',
        ...
        'CHIP_FIND','http://localhost:8081/api/v1/detail', true),
       (6, 'АО Тинькофф', '044525974', '30101810145250000974', 'OOO Техэнерго', '124482', 'Москва',
        ...
        'FREE_CHIPS','http://localhost:8082/api/v1/detail', true),
       (7, 'АО Тинькофф', '044525974', '30101810145250000974', 'OOO ЧипФайнд', '124482', 'Санкт-Петербург',
        ...
        'RADIO_COMPONENT','http://localhost:8083/api/v1/detail', true),
...
-- оплату (и в запросах продавцу) если вдруг покупатель хочет чтобы счет выставлялся на юр. лицо,
-- и вместо ФИО, была указана юридическая информация о покупателе.
...
-- уменьшать, при добавлении товара происходит поиск по корзине по наименованию товара и
-- id продавца если есть
...
-- payment_gate_uuid - uuid заказа в платежной системе сгенерированный платежным шлюзом
-- при оплате онлайн
...
-- request_item - таблица с товарами в запросе
-- product_id  - айди товара, передается из товара в корзине запросрв, чтобы не генерирвоать
-- лишний раз
-- seller_request_id  - айди запроса продавца, к которому относится товар
-- seller_id - айди продавца
...
-- min_quantity -  миннимальное количество для заказа
-- first_price - цена, которая дейтсвует от min_quantity единиц товара
