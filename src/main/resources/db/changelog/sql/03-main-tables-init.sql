...
-- busines_address - юр. адрес продавца полностью (обяз. поле, не для ИП, г. Москва, Каширское шоссе, д55,к5) 1
...
-- оплату, если вдруг покупатель хочет чтобы счет выставлялся на юр. лицо, и вместо ФИО, была указана
...
-- busines_address - юр.адрес
-- inn, kpp - ИНН и КПП
-- phone_number, email - номер телефона и почта, куда отправлять ссылку для получения счета
-- при заполнении данных и сохранении у него минимально должно быть указано 2 поля
-- company_name, inn
